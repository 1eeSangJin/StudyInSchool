...
use \PDO;
    class memberDao {
        ...
        function insertUser($userId, $userPw, $userNick, $userName, $sex, $userPhone, $affNum){
             ...
             *********************************************************/
            try{
                $sql = "insert into users(id, userPw, userNick, userName, sex, userPhone, affNum) values(:id, :userPw, :userNick, :userName, :sex, :userPhone, :affNum)";
                $pstmt = $this->db->prepare($sql);

                $pstmt->bindValue(":id", $userId, PDO::PARAM_STR);
        ...
        function updateUser($userPw, $userNick, $userPhone, $affNum, $userId){
            try{
                ...
                $sql2 = "update users set userPw=:userPw, userNick=:userNick, userPhone=:userPhone, affNum=:affNum where id=:id";
        ...
        function getAff($userId){
            try{
                $sql = "select u.id, a.affName from users u, affiliation a where u.affNum = a.affNum and u.id=:id";
                $pstmt = $this->db->prepare($sql);
