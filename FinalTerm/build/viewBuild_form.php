...
<body>
  ...
  <?php
        ...
        $dao = new boardDao();
        $msgs = $dao->getBuild($num);
        $comments = $dao->getAllCommentBuild($num);
        $count = $dao->countCommentBuild($num);
        // $dao->incresBuildHits($num);
  ?>
      ...
      <div>
          ...
          <div id = "contents">
            <span style = "float:right;">
              <a href = "deleteBuild.php?num=<?= $msgs['num'] ?>&page=<?= $page ?>" onclick = "return confirm('정말 삭제하시겠습니까?')" class = "ui secondary button">삭제</a>
              <button class = 'ui secondary button' onclick = "location.href='modifyBuild_form.php?num=<?=$msgs['num'] ?>&page=<?= $page ?>'">수정</button>
              <button class = 'ui secondary button' onclick = "location.href='buildBoard.php?page=<?= $page?>'">목록</button>
            </span>
            <br><br>
          <div>
            <?= $msgs['content'] ?>
