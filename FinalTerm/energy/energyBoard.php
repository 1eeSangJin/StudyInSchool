...
<body>
  ...
  <?php
        ...
        $NumOfEnergy = $dao->getNumOfEnergy();

        if($NumOfEnergy > 0){
          $numPages = ceil($NumOfEnergy / NUM_LINES);
          ...
          $msgs = $dao->getAllenergy($start, NUM_LINES);
  ...
  <header>
    ...
    <div class = "column" id = "content">
      ...
      <?php if($NumOfEnergy > 0) : ?>
          ...
          <?php error_reporting(0) ?>
            <?php foreach($msgs as $row) : ?>                         <!-- 리턴받은 $msgs를 $row라는 변수에 연관배열로 받는다. 끝까지 받으면 종료됨 -->
              <tr>
                ...
                <td>
                  <a href = "viewEnergy_form.php?num=<?= $row['num'] ?>&page=<?= $page ?>"> <!-- 게시글 상세보기 링크를 단다. -->
                    <?= $row['title'] ?>[<?= $count = $dao->countCommentEnergy($row['num']); ?>]                        <!-- title에 있는 값을 출력한다. -->
                  </a>
                </td>
                <td>
        ...
        <br>
        <?php if($firstLink > 1) : ?>
          <a href="<?= bdUrl('energyBoard.php', 0, $page - NUM_PAGE_LINKS) ?>"><</a>&nbsp;
        <?php endif ?>

        <?php for($i = $firstLink; $i <= $lastLink; $i++) : ?>
          <?php if($i == $page) : ?>
            <a href="<?= bdUrl('energyBoard.php', 0 , $i) ?>"><b><?= $i ?></b></a>&nbsp;
          <?php else : ?>
            <a href="<?= bdUrl('energyBoard.php', 0 , $i) ?>"><?= $i ?></a>&nbsp;
          <?php endif ?>
        <?php endfor ?>
      ...
      <?php endif ?>
        ...
        <div style="float:right;">
          <button type = 'button' class = 'ui secondary button' onclick = location.href='writeEnergy_form.php'>글쓰기</button>
        </div>
    </div>
